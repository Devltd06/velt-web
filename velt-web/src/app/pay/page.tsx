// src/app/signup/pay/page.tsx  (or src/app/pay/page.tsx — use the same path you already have)
"use client";

import { useEffect, useMemo, useState } from "react";
import { useSearchParams, useRouter } from "next/navigation";

export default function PayDebug() {
  ...
  const addLog = (m: any) => {
    const s = typeof m === "string" ? m : (m instanceof Error ? `${m.message}\n${m.stack}` : JSON.stringify(m, null, 2));
  ...
  useEffect(() => {
    ...
    s.onload = () => {
      ...
      addLog("Paystack script loaded (onload). window.PaystackPop available? " + !!(window as any).PaystackPop);
    };
    s.onerror = (ev) => {
  ...
  // env key & derived amounts (hooks at top)
  const PAYSTACK_PUBLIC_KEY = process.env.NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY || "";
  useEffect(() => {
    const masked = PAYSTACK_PUBLIC_KEY ? `${PAYSTACK_PUBLIC_KEY.slice(0, 8)}...${PAYSTACK_PUBLIC_KEY.slice(-6)}` : "<EMPTY>";
  ...
  const reference = useMemo(() => `VELT-${Date.now()}-${Math.floor(Math.random() * 1e6)}`, []);
    ...
    return (
      <div className="min-h-screen flex items-start justify-center bg-gray-900 text-white p-8">
        <div className="max-w-2xl w-full">
          ...
          <p className="mb-4">This page requires payment data passed in the `data` query param. Go back to the subscription page and choose a plan.</p>
          <button onClick={() => router.back()} className="bg-blue-600 px-4 py-2 rounded">Go Back</button>

          <div className="mt-6 bg-black/40 p-4 rounded">
            <h3 className="font-semibold mb-2">Debug logs</h3>
            <div style={{ maxHeight: 300, overflow: 'auto', fontSize: 12, whiteSpace: 'pre-wrap' }}>
  ...
  const { email, plan, role, priceUSD, priceGHS: pghs, username, fullName } = payData;
  ...
  const handlePay = async () => {
    try {
      ...
      addLog({ PAYSTACK_PUBLIC_KEY_masked: PAYSTACK_PUBLIC_KEY ? `${PAYSTACK_PUBLIC_KEY.slice(0,8)}...${PAYSTACK_PUBLIC_KEY.slice(-6)}` : "<EMPTY>" });
      addLog({ scriptLoaded, PaystackPresent: !!(window as any).PaystackPop, amountInPesewas, reference });
      ...
      // wrap the setup in try/catch for synchronous errors
      let handler;
      try {
            ...
            (async () => {
              try {
                const res = await fetch("/api/subscription", {
                  ...
                  body: JSON.stringify({ email, role, reference: resp.reference }),
                });
      ...
      // open the iframe (wrap in try/catch)
      try {
    ...
    <div className="min-h-screen flex items-start justify-center bg-gray-900 text-white p-8 space-y-6">
      <div className="max-w-2xl w-full space-y-6">
        <div className="bg-gray-800 rounded p-6">
          ...
          <p className="text-gray-300">{username} {fullName ? `(${fullName})` : ""}</p>
          <p className="text-gray-300">Plan: {plan}</p>
          <p className="text-gray-300 mb-3">${Number(priceUSD).toFixed(2)} (≈ GHS {Number(pghs).toFixed(2)})</p>

          <button onClick={handlePay} className="bg-blue-600 px-6 py-3 rounded font-semibold">Pay with Paystack</button>
        </div>

        <div className="bg-black/40 p-4 rounded">
          <h3 className="font-semibold mb-2">Debug logs (latest at bottom)</h3>
          <div style={{ maxHeight: 300, overflow: 'auto', fontSize: 12, whiteSpace: 'pre-wrap' }}>
            {logs.length === 0 ? <div className="text-gray-400">No logs yet — click Pay to start.</div> :
              logs.map((l, i) => <div key={i} className="mb-1">{l}</div>)
            }
          </div>
        </div>
