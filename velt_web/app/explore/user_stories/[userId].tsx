...
import { getCachedUserStories } from '@/lib/store/prefetchStore';
...
export function UserStoriesView({ initialUserId: initialUserIdProp = null, onClose }: UserStoriesViewProps) {
  ...
  // callout modal state (replaces previous shooting flow)
  ...
  // legacy shoot sheet state (kept for compatibility)
  ...
  async function fetchAllStories() {
    // Check for cached data first for instant load
    const cachedGroups = getCachedUserStories(initialUserId || undefined);
    if (cachedGroups && cachedGroups.length > 0) {
      // Use cached data immediately
      const arr = cachedGroups.map(g => ({
        userId: g.userId,
        profile: g.profile as ProfileShort,
        stories: g.stories.map(s => ({
          ...s,
          profiles: s.profiles as ProfileShort | null,
        })) as StoryItem[],
      }));
      ...
      setLoading(false);
      return; // Use cached data, skip fresh fetch
    }
    ...
    try {
      ...
      try {
        const res = await supabase.from("stories").select(trySelectWithCaption).eq("is_deleted", false).order("created_at", { ascending: false });
        if ((res as any).error) throw (res as any).error;
        data = (res as any).data || [];
      } catch (err) {
        const safeSelect = `id, user_id, media_url, media_type, duration, created_at, is_deleted, expire_at, is_hd,
           profiles:profiles!stories_user_id_fkey(id, username, full_name, avatar_url)`;
        const res2 = await supabase.from("stories").select(safeSelect).eq("is_deleted", false).order("created_at", { ascending: false });
  ...
  async function startOrOpenDM(targetId: string, targetName?: string, targetAvatar?: string) {
    try {
      ...
      if (partErr) {
        console.warn('participants.insert failed w/ accepted, trying fallback', partErr);
        try {
      ...
      // insert explicit invite so recipient gets prompted to accept/abort
  ...
  async function addComment(storyId: string, content: string, parentId?: string | null) {
    ...
    const payload: Record<string, any> = { story_id: storyId, user_id: myId, content };
    if (parentId) payload.parent_id = parentId;

    const insertRes = await supabase.from("story_comments").insert(payload).select("id").single();
  ...
  async function retryComment(localId: string) {
    ...
    try {
      ...
      const saved = await addComment(pending.story_id, pending.content, pending.parent_id ?? undefined);
  ...
  async function openShootModalFor(target?: StoryItem | null) {
    try {
      ...
      if (followingIds.length) {
        ...
        setShootFollowing((profs || []).map((p: any) => ({ id: p.id, full_name: p.full_name, username: p.username, avatar_url: p.avatar_url })));
      } else {
        setShootFollowing([]);
      }
      ...
      setShootSuggestions((suggestions || []).map((p: any) => ({ id: p.id, full_name: p.full_name, username: p.username, avatar_url: p.avatar_url })));
    } catch (err) {
      ...
      for (const tid of ids) {
        try {
          ...
          const { error } = await supabase
            .from("messages")
            .insert({ conversation_id: cid, sender_id: myId, content: shootTarget.caption ?? "", media_url: mediaUrl, media_type: shootTarget.media_type });
  ...
  const handleShootSendAll = async () => {
  ...
  const findOrCreateDM = async (targetId: string) => {
    try {
      ...
      if (partErr) {
        console.warn("participant insert failed with accepted flag", partErr);
        try {
          const { error: fbErr } = await supabase.from('conversation_participants').insert([
      ...
      // invitations are prompt-driven via conversation_participants.accepted; do not insert messages
      return cid;
    } catch (err) {
  ...
  const sendCalloutToRecipients = async (overrideIds?: string[]) => {
    if (!shootTarget) {
      Alert.alert("No target", "No story selected to callout.");
      return;
    }
    const ids = Array.isArray(overrideIds) && overrideIds.length ? overrideIds : Array.from(selectedCalloutRecipients);
    if (!ids.length) {
      Alert.alert("Select recipients", "Pick at least one person to send to.");
      return;
    }
    ...
    try {
      ...
      for (const targetId of ids) {
        try {
          ...
          if (!cid) { results.push({ id: targetId, success: false, error: 'no-convo' }); continue; }
          const { data: inserted, error: insertErr } = await supabase.from('messages').insert({ conversation_id: cid, sender_id: myId, content: shootTarget.caption ?? '', media_url: mediaUrl, media_type: mediaType, story_id: shootTarget.id }).select('id').single();
          if (insertErr || !inserted?.id) results.push({ id: targetId, success: false, error: String(insertErr?.message ?? insertErr) });
          else results.push({ id: targetId, success: true });
        } catch (err) {
  ...
  const handleSendAll = async () => {
    const ids = calloutCandidates.map((p) => p.id).filter((id): id is string => !!id);
    if (!ids.length) {
      Alert.alert('No recipients', 'There are no users available here yet.');
      return;
    }
    setSelectedCalloutRecipients(new Set(ids));
    await sendCalloutToRecipients(ids);
  };
  ...
  function renderCalloutModal() {
    ...
    return (
      <Modal visible={calloutModalVisible} transparent animationType="none" onRequestClose={() => setCalloutModalVisible(false)}>
        <TouchableWithoutFeedback onPress={() => setCalloutModalVisible(false)}>
          <View style={styles.sheetOverlay} />
        </TouchableWithoutFeedback>
        <Animated.View style={[styles.bottomSheetContainer, { backgroundColor: '#000', paddingTop: 8, borderTopLeftRadius: 16, borderTopRightRadius: 16, height: calloutExpanded ? Math.max(WINDOW_H * 0.6, WINDOW_H - 80) : undefined }]}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 12 }}>
            ...
            <TouchableOpacity onPress={() => setCalloutExpanded((v) => !v)} style={{ padding: 8, marginRight: 6 }}>
              <Ionicons name={calloutExpanded ? 'chevron-down' : 'chevron-up'} size={18} color={'rgba(255,255,255,0.6)'} />
            </TouchableOpacity>
          </View>
          <Text style={styles.sheetTitle}>Send Callout</Text>
          <Text style={[styles.sheetDesc, { color: 'rgba(255,255,255,0.7)', marginBottom: 6 }]}>Pick followers or suggested recipients to share this content with.</Text>

          <View style={{ flexDirection: 'row', alignItems: 'center', marginVertical: 8 }}>
            <TouchableOpacity style={styles.sheetBtn} onPress={() => { if (currentProfile?.id) { const ids = (calloutCandidates || []).map((c) => c?.id).filter(Boolean) as string[]; setSelectedCalloutRecipients(new Set(ids)); } }}>
              <Text style={[styles.sheetText, { color: '#fff' }]}>Select All Followers</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.sheetBtn} onPress={() => { setSelectedCalloutRecipients(new Set()); }}>
              <Text style={[styles.sheetText, { color: '#fff' }]}>Clear</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={{ flex: 1, paddingVertical: 6 }}>
            {calloutCandidates.length === 0 ? (
              <View style={{ alignItems: 'center', paddingVertical: 16 }}>
                ...
                <Text style={{ color: 'rgba(255,255,255,0.6)', marginTop: 8 }}>No followers found.</Text>
              </View>
            ) : (
              <FlatList
                ...
                renderItem={({ item }) => {
                  ...
                  return (
                    <TouchableOpacity onPress={() => toggleCalloutRecipient(id)} style={{ marginRight: 8, width: 80 }}>
                      {item?.avatar_url ? (
                        <Image source={{ uri: item.avatar_url }} style={{ width: 64, height: 64, borderRadius: 12 }} />
                      ) : (
                        <View style={{ width: 64, height: 64, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.04)', alignItems: 'center', justifyContent: 'center' }}>
                          <Text style={{ color: '#fff' }}>{(item?.full_name || item?.username || 'U').slice(0,2).toUpperCase()}</Text>
                        </View>
                      )}
                      <Text numberOfLines={1} style={{ color: '#fff', fontSize: 12, marginTop: 6 }}>{display}</Text>
                      <Text style={{ color: selected ? '#4ea8ff' : 'rgba(255,255,255,0.6)', fontWeight: '700' }}>{selected ? 'Selected' : ''}</Text>
                    </TouchableOpacity>
                  );
                }}
              />
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 8, paddingBottom: 20 }}>
            <TouchableOpacity style={[styles.sheetBtn, { justifyContent: 'center', borderRadius: 999, paddingVertical: 10, paddingHorizontal: 18, backgroundColor: 'transparent', borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' }]} onPress={() => setCalloutModalVisible(false)}>
              <Text style={[styles.sheetText, { color: '#fff' }]}>Cancel</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.sheetBtn, { justifyContent: 'center', backgroundColor: '#4ea8ff', paddingHorizontal: 18, paddingVertical: 10, borderRadius: 999 }]} onPress={() => { const recips = Array.from(selectedCalloutRecipients); if (recips.length === 0) { Alert.alert('No recipients', 'Please select at least one recipient'); return; } sendCalloutToRecipients(recips); }}>
              <Text style={{ color: '#fff', fontWeight: '800' }}>{calloutLoading ? 'Sending...' : 'Send Callout'}</Text>
            </TouchableOpacity>
          </View>
            </Animated.View>
          </Modal>
  ...
  async function loadSocialStateForActive() {
    ...
    try {
      ...
      if (s.id) {
        const myId = await getCurrentUserId();
        if (myId) {
          const { data: liked } = await supabase.from("story_likes").select("id").eq("story_id", s.id).eq("user_id", myId).maybeSingle();
      ...
      Alert.alert("Delete story", "Are you sure you want to delete this story?", [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              // soft-delete
              const { error } = await supabase.from("stories").update({ is_deleted: true }).eq("id", s.id);
              if (error) throw error;
