-- Migration: store raw latitude/longitude for location posts
-- Adds latitude/longitude columns and updates the insert_location_post RPC

alter table if exists public.location_posts add column if not exists latitude double precision;
alter table if exists public.location_posts add column if not exists longitude double precision;

-- Replace insert_location_post to accept raw geo coordinates and use them
...
declare
  ...
  -- base_x and base_y are normalized 0..1 coordinates used for layout
...
begin
  ...
  -- Prefer explicit normalized p_lat/p_lng if provided, otherwise compute from provided geo coords.
  ...
  insert into public.location_posts(user_id, place, country, images, avatar_url, author_display_name, caption, position, latitude, longitude, created_at)
    values (
      v_user_id,
      p_place,
      p_country,
...
grant execute on function public.insert_location_post(text, text, text[], text, text, double precision, double precision, double precision, double precision) to authenticated;
